...
@section('title', 'Statusy dokumentów')
    ...
    <div class="space-y-8">
        ...
        <div class="rounded-[2rem] bg-white p-8 shadow-xl ring-1 ring-slate-200">
            <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <p class="text-sm uppercase tracking-[0.32em] text-slate-500">Statusy dokumentów</p>
                    <h1 class="mt-3 text-3xl font-semibold text-slate-950">Zarządzaj statusami</h1>
                    <p class="mt-2 text-sm text-slate-600">Dodaj statusy, które będziesz mógł wybierać w formularzu dokumentu.</p>
                </div>
            </div>
            ...
            <div class="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
                <section class="space-y-6">
                    @if($statuses->isEmpty())
                        <div class="rounded-[1.75rem] border border-slate-200 bg-slate-50 p-8 text-center text-slate-600">
                            Brak statusów. Dodaj pierwszy status po prawej stronie.
                        </div>
                    @else
                        <div class="space-y-3">
                            @foreach($statuses as $status)
                                <div class="flex items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('documents.statuses.edit', $status) }}" class="rounded-2xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-900 transition hover:border-slate-400 hover:bg-slate-100">Edytuj</a>
                ...
                <aside class="rounded-[2rem] border border-slate-200 bg-slate-50 p-6">
                    @if(isset($editStatus))
                        <h2 class="text-xl font-semibold text-slate-950">Edytuj status</h2>
                        <form method="POST" action="{{ route('documents.statuses.update', $editStatus) }}" class="mt-6 space-y-4">
                            @csrf
                            @method('PUT')
                            <div>
                                <label class="block text-sm font-medium text-slate-700">Nazwa statusu</label>
                            ...
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                ...
                                <a href="{{ route('documents.statuses.index') }}" class="inline-flex justify-center rounded-2xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-900 transition hover:bg-slate-100">Anuluj</a>
                            </div>
                        </form>
                    @else
                        <h2 class="text-xl font-semibold text-slate-950">Dodaj status</h2>
                        <form method="POST" action="{{ route('documents.statuses.store') }}" class="mt-6 space-y-4">
                            ...
                            <button type="submit" class="inline-flex justify-center rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-700">Dodaj status</button>
                        </form>
                    @endif
                </aside>
            </div>
