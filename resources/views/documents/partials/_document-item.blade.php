...
<div class="border-b border-slate-200 last:border-b-0 {{ $rowClass }}" style="margin-left: {{ ($level ?? 0) * 24 }}px;">
    <div class="flex items-center gap-3 px-6 py-4 transition-colors {{ $hoverClass }}"
        ...
        <div class="flex gap-2 flex-shrink-0">
            ...
            <a href="{{ $canManageDocuments ? route('documents.edit', ['document' => $document, 'return_url' => url()->full()]) : route('documents.preview_public', $document) }}" class="text-slate-500 hover:text-slate-700 p-1">
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
